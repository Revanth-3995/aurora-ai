...
_backend_dir = Path(__file__).resolve().parent.parent   # .../agentic_ai_project/backend
_root_dir  = _backend_dir.parent                        # .../agentic_ai_project
...
def _resolve_path(value: Optional[str]) -> Optional[str]:
    ...
    if p.is_absolute():
        return str(p)
    # Resolve relative to root where the .env and credential files live
    resolved = _root_dir / p
    return str(resolved)
